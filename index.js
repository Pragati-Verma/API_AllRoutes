...
app.get("/random", (req,res) => {
  ...
  console.log(jokes[randomJokeIndex]);
...
app.get("/jokes/:id", (req,res) => {
  ...
  const result = jokes.find((jokes) => jokes.id === id);
  console.log(result);
  res.json(result);
});
...
app.get("/filter", (req,res) => {
  ...
  const jokesResult = jokes.filter((jokes) => jokes.jokeType === type);
  console.log(jokesResult);
  res.json(jokesResult);
});
...
app.patch("/jokes/:id", (req,res) => {
  const id = parseInt(req.params.id);
  const currentJoke = jokes.find((jokes) => jokes.id === id);
  const index = jokes.findIndex((jokes) => jokes.id === id);
  const updatedJoke = {
    id: id,
...
app.delete("/jokes/:id", (req,res) => {
  const id = parseInt(req.params.id);
  const findJoke = jokes.findIndex((jokes) => jokes.id === id);
  if(findJoke > -1) {
    jokes.splice(findJoke,1);
    res.sendStatus(200);
  }
  else{
    res
...
app.delete("/all", (req,res) => {
  ...
  if(userKey === masterKey) {
    ...
    res
    .status(404)
    .json({ error: "The master key you entered is invalid. Cannot delete all jokes..."});
  }
})
